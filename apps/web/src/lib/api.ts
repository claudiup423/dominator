...
export const models = {
  ...
  update: (id: string, data: Record<string, unknown>) =>
    request(`/api/models/${id}`, { method: 'PATCH', body: JSON.stringify(data) }),
  promote: (id: string) =>
...
export const trainingRuns = {
  ...
  start: (data: { model_id: string; config_json?: Record<string, unknown> }) =>
    request('/api/training-runs/start', { method: 'POST', body: JSON.stringify(data) }),
  stop: (id: string) => request(`/api/training-runs/${id}/stop`, { method: 'POST' }),
  checkpoints: (id: string) => request(`/api/training-runs/${id}/checkpoints`),
  stream: (id: string) => new EventSource(`${API_BASE}/api/stream/training-runs/${id}`),
};

// ─── Evals ────────────────────────────────────────────────────────
export const evals = {
  suites: () => request('/api/eval-suites'),
  createSuite: (data: { name: string; definition_json?: Record<string, unknown> }) =>
  ...
  compare: (baseId: string, candidateId: string) =>
    request(`/api/evals/compare?base_checkpoint_id=${baseId}&candidate_checkpoint_id=${candidateId}`),
...
export const sessions = {
  ...
  start: (data: { mode: string; difficulty: string; opponent_style: string }) =>
    request('/api/sessions/start', { method: 'POST', body: JSON.stringify(data) }),
  addEvent: (id: string, data: { t_ms: number; type: string; payload_json?: Record<string, unknown> }) =>
    request(`/api/sessions/${id}/event`, { method: 'POST', body: JSON.stringify(data) }),
  end: (id: string, data: { score_json: Record<string, number> }) =>
    request(`/api/sessions/${id}/end`, { method: 'POST', body: JSON.stringify(data) }),
